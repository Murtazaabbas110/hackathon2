...
function StatPill({ label, value, tone = "default" }) {
  ...
  return (
    <div className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ${toneClasses[tone] || toneClasses.default}`}>
...
export function IntelligenceOverview({ analysis }) {
  ...
  return (
    <Card className="border-slate-700/70 bg-slate-900/60">
      <CardHeader>
        <CardTitle className="text-sm font-semibold tracking-tight text-slate-50">Overview</CardTitle>
        <CardDescription className="text-xs text-slate-400">
          Snapshot of the project intelligence extracted from the client message.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 text-sm">
        ...
        <div className="flex flex-wrap gap-2">
          <StatPill label="Complexity" value={analysis.complexity || "N/A"} tone={complexityTone} />
          <StatPill label="Readiness" value={`${analysis.readiness ?? "?"}%`} tone={readinessTone} />
          <StatPill label="Requirements" value={requirementsCount} />
          <StatPill label="Risks" value={risksCount} tone={risksCount ? "danger" : "success"} />
          <StatPill label="Ambiguities" value={ambiguitiesCount} tone={ambiguitiesCount ? "warning" : "success"} />
        </div>

        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Objectives</h3>
            {analysis.objectives?.length ? (
              ...
              <p className="text-xs text-slate-500">No objectives were extracted.</p>
            )}
          </div>

          <div className="space-y-1">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Target users</h3>
            {analysis.targetUsers?.length ? (
              ...
              <p className="text-xs text-slate-500">No explicit target users were identified.</p>
            )}
          </div>
        </div>
      </CardContent>
