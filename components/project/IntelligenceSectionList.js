...
function ItemRow({ title, description, meta, sourceText }) {
  return (
    <div className="rounded-md border border-slate-800/70 bg-slate-950/40 p-3 text-xs">
        ...
        <div className="space-y-1">
          ...
          {sourceText && (
            <p className="mt-1 border-l border-slate-700/70 pl-2 text-[11px] text-slate-400">
              From client message: <span className="italic">“{sourceText}”</span>
            </p>
          )}
        </div>
        {meta && title && (
          <span className="shrink-0 rounded-full bg-slate-900 px-2 py-0.5 text-[10px] text-slate-400">
            {meta}
          </span>
        )}
      </div>
...
function normalizeItem(item) {
  if (!item || typeof item !== "object") {
    return { title: null, description: String(item || ""), meta: null, sourceText: null };
  }
...
export function IntelligenceSectionList({ title, description, emptyLabel, items }) {
  ...
  return (
    <Card className="border-slate-700/70 bg-slate-900/60">
      <CardHeader className="flex flex-row items-center justify-between gap-2">
        <div>
          <CardTitle className="text-sm font-semibold tracking-tight text-slate-50">{title}</CardTitle>
          {description && (
